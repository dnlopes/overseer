...
func TestPaneDetector_AgentType_ReturnsOpenCode(t *testing.T) {
	d := NewPaneDetector(nil)
	if got := d.AgentType(); got != domain.AgentTypeOpenCode {
		t.Fatalf("AgentType() = %q, want %q", got, domain.AgentTypeOpenCode)
	}
}
...
func TestPaneDetector_Detect_RunningFixtures(t *testing.T) {
	...
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if status.Source != "opencode/pane-parser" {
				t.Fatalf("Detect(%s).Source = %q, want %q", name, status.Source, "opencode/pane-parser")
			}
			if status.Reason == "" {
...
func TestPaneDetector_Detect_WaitingFixtures(t *testing.T) {
	cases := []string{
		"waiting_bash_permission.txt",
		"waiting_edit_permission.txt",
		"waiting_agent_question.txt",
	}
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			status := detectFromFixture(t, name)
...
func newDetectorWithStub(t *testing.T) (*PaneDetector, *mocks.MockTmuxAdapter, domain.Session) {
	...
	sess := domain.Session{ID: uuid.New(), AgentType: domain.AgentTypeOpenCode}
	return d, tmux, sess
}
