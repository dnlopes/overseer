...
//
//	OVERSEER_LIVE_TMUX_ID=<agent-tmux-name> go test -tags=live_tmux \
//	    ./internal/adapters/secondary/agentstatus/opencode/ \
...
func TestPaneDetector_Detect_LiveTmux(t *testing.T) {
	...
	if tmuxIDFromEnv == "" {
		t.Skip("set OVERSEER_LIVE_TMUX_ID=<tmux-name>-agent to run this test")
	}
	...
	sess := domain.Session{
		Name:      "live-verify",
		AgentType: domain.AgentTypeOpenCode,
		TmuxName:  strings.TrimSuffix(tmuxIDFromEnv, "-agent"),
	}
